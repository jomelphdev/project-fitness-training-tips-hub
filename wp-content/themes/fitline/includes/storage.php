...
if ( ! function_exists( 'fitline_storage_get' ) ) {
	/**
	 ...
	 */
	function fitline_storage_get( $var_name, $default = '' ) {
		global $FITLINE_STORAGE;
		return isset( $FITLINE_STORAGE[ $var_name ] ) ? $FITLINE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'fitline_storage_set' ) ) {
	/**
	 ...
	 */
	function fitline_storage_set( $var_name, $value ) {
		global $FITLINE_STORAGE;
		$FITLINE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'fitline_storage_empty' ) ) {
	/**
	 ...
	 */
	function fitline_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $FITLINE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $FITLINE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $FITLINE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fitline_storage_isset' ) ) {
	/**
	 ...
	 */
	function fitline_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $FITLINE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $FITLINE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $FITLINE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fitline_storage_unset' ) ) {
	/**
	 ...
	 */
	function fitline_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $FITLINE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $FITLINE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $FITLINE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fitline_storage_inc' ) ) {
	/**
	 ...
	 */
	function fitline_storage_inc( $var_name, $value = 1 ) {
		global $FITLINE_STORAGE;
		if ( empty( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = 0;
		}
		$FITLINE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'fitline_storage_concat' ) ) {
	/**
	 ...
	 */
	function fitline_storage_concat( $var_name, $value ) {
		global $FITLINE_STORAGE;
		if ( empty( $FITLINE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'fitline_storage_get_array' ) ) {
	/**
	 ...
	 */
	function fitline_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $FITLINE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $FITLINE_STORAGE[ $var_name ][ $key ] ) ? $FITLINE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'fitline_storage_set_array' ) ) {
	/**
	 ...
	 */
	function fitline_storage_set_array( $var_name, $key, $value ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FITLINE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'fitline_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function fitline_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $FITLINE_STORAGE[ $var_name ][ $key ] ) ) {
			$FITLINE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$FITLINE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$FITLINE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'fitline_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function fitline_storage_merge_array( $var_name, $key, $value ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FITLINE_STORAGE[ $var_name ] = array_merge( $FITLINE_STORAGE[ $var_name ], $value );
		} else {
			$FITLINE_STORAGE[ $var_name ][ $key ] = array_merge( $FITLINE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fitline_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function fitline_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fitline_array_insert_after( $FITLINE_STORAGE[ $var_name ], $after, $key );
		} else {
			fitline_array_insert_after( $FITLINE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fitline_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function fitline_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fitline_array_insert_before( $FITLINE_STORAGE[ $var_name ], $before, $key );
		} else {
			fitline_array_insert_before( $FITLINE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fitline_storage_push_array' ) ) {
	/**
	 ...
	 */
	function fitline_storage_push_array( $var_name, $key, $value ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $FITLINE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $FITLINE_STORAGE[ $var_name ][ $key ] ) ) {
				$FITLINE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $FITLINE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fitline_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function fitline_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $FITLINE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $FITLINE_STORAGE[ $var_name ] ) && is_array( $FITLINE_STORAGE[ $var_name ] ) && count( $FITLINE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $FITLINE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $FITLINE_STORAGE[ $var_name ][ $key ] ) && is_array( $FITLINE_STORAGE[ $var_name ][ $key ] ) && count( $FITLINE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $FITLINE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'fitline_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function fitline_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FITLINE_STORAGE[ $var_name ][ $key ] ) ) {
			$FITLINE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$FITLINE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'fitline_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function fitline_storage_concat_array( $var_name, $key, $value ) {
		global $FITLINE_STORAGE;
		if ( ! isset( $FITLINE_STORAGE[ $var_name ] ) ) {
			$FITLINE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FITLINE_STORAGE[ $var_name ][ $key ] ) ) {
			$FITLINE_STORAGE[ $var_name ][ $key ] = '';
		}
		$FITLINE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'fitline_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function fitline_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $FITLINE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FITLINE_STORAGE[ $var_name ] ) ? $FITLINE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FITLINE_STORAGE[ $var_name ] ) ? $FITLINE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'fitline_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function fitline_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $FITLINE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $FITLINE_STORAGE[ $var_name ]->$prop ) ? $FITLINE_STORAGE[ $var_name ]->$prop : $default;
	}
}
