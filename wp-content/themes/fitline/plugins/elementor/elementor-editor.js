...
jQuery( document ).ready(
	function() {
		...
		fitline_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="fitline_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.fitline_post_editor' );
						if ( link.length > 0 ) {
							fitline_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.fitline_post_editor', function(e) {
						fitline_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('fitline_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function fitline_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
