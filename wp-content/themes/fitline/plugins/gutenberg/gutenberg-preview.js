...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			fitline_create_observer( 'fitline-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				fitline_gutenberg_editor_init();
			} );
		} else {
		 	fitline_gutenberg_editor_init();
		}
	...
	function fitline_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'fitline_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'fitline_inited' ) ) {
	...
	// Init on page load
	function fitline_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = fitline_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + FITLINE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + FITLINE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in FITLINE_STORAGE['override_classes'] ) {
				$obj.addClass( FITLINE_STORAGE['override_classes'][i].replace( '%s', FITLINE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( fitline_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && FITLINE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'fitline_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			fitline_remove_observer( 'fitline-check-editor-styles-wrapper' );
			fitline_create_observer( 'fitline-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'fitline_inited' );
	}
} );
