...
if ( apply_filters( 'fitline_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$fitline_with_featured_image = fitline_is_with_featured_image( $fitline_post_header, array( 'with_gallery' ) );
	// Post title and meta
	ob_start();
	...
	if ( strpos( $fitline_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $fitline_with_featured_image ) {
                echo ' with_featured_image' . ( false && fitline_get_theme_option( 'single_parallax' ) == 0 ? ' fitline-full-height' : '' );
            }
		?>">
			<?php
