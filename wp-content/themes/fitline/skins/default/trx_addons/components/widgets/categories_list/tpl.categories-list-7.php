<?php
/**
 * The "Grid 1" template to display the categories list
 *
...
if($args['number'] > count( $args['categories'] )) {
	$args['number'] = count( $args['categories'] );
}
...
// Widget body
$title_link_text = esc_html__('View All','fitline');
if(isset($args['taxonomy']) && 'product_cat' == $args['taxonomy']) {
	$title_link_text = esc_html__('Shop Now','fitline');
}

?>
	<div class="categories_list categories_list_style_<?php echo esc_attr($style); ?> <?php echo esc_attr('count_'. $args['number']); ?>">
		<?php
		if ($args['slider']) {
			...
			?><div class="slider-slide swiper-slide"><?php
				...
				?><div class="<?php echo esc_attr(trx_addons_get_column_class(1, $columns, $columns_tablet, $columns_mobile)); ?>"><?php
					...
					<div class="categories_list_item">
						...
						?><div class="categories_list_image post_featured_bg hover_link"><?php
						$src = empty($image)
							? trx_addons_get_no_image()
							: trx_addons_add_thumb_size($image, fitline_get_thumb_size('full'));
						$attr = trx_addons_getimagesize($src);
						?>
							<div class="post_thumb bg_in <?php echo fitline_add_inline_css_class('background-image: url('. esc_url($src).');'); ?>"></div>
						</div><?php
						?>
						<div class="categories_list_info_wrap">
							<div class="categories_list_info">
								...
								<a href="<?php echo esc_url($cat_link); ?>" class="categories_link_more"><?php fitline_show_layout($title_link_text); ?></a>
							</div>
						</div>
