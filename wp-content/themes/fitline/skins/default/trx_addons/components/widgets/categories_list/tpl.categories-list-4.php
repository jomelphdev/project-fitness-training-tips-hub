<?php
/**
 * The "Extra 1" template to display the categories list
 *
		...
		?>"><?php
			...
			?><div class="slider-slide swiper-slide"><?php
				...
				?><div class="<?php echo esc_attr(trx_addons_get_column_class(1, $columns, $columns_tablet, $columns_mobile)); ?>"><?php
					...
					?>
					<div class="categories_list_item tilt_action_cat_list">
						<?php
						...
						$src = empty($image)
							...
							: trx_addons_add_thumb_size($image, fitline_get_thumb_size($columns > 2 ? 'big' : 'huge'));
						$attr = trx_addons_getimagesize($src);
						?><img src="<?php echo esc_url($src); ?>" <?php if (!empty($attr[3])) trx_addons_show_layout($attr[3]); ?> alt="<?php esc_attr_e('Category image', 'fitline'); ?>">
						</div><?php
						?>
