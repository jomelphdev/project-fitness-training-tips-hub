<?php
/**
 * The style "creative" of the Services item
 *
...
if ( has_post_thumbnail() ) {
	$image = trx_addons_get_attachment_url( 
											...
											! empty( $args['thumb_size'] )
												? $args['thumb_size']
												: apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('masonry-big'), 'services-creative')
											);
}
...
<div data-post-id="<?php the_ID(); ?>" <?php post_class( apply_filters( 'trx_addons_filter_services_item_class',
			'sc_services_item sc_item_container post_container tilt_action'
			. (empty($link) ? ' no_links' : '')
