<?php
/**
 * The style "unusual" of the Services
 *
        ...
        if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters( 'trx_addons_filter_args_featured', array(
																...
																'thumb_size' => ! empty( $args['thumb_size'] )
                                                                                    ? $args['thumb_size']
                                                                                    : apply_filters( 'trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] >= 2 ? 'square' : 'big'), 'services-unusual', $args ),
            ...
            if (!empty($svg)) {
                ...
                ?><img class="sc_icon_as_image"
                       src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'fitline'); ?>"<?php echo(!empty($attr[3]) ? ' ' . trim($attr[3]) : ''); ?>><?php
            }
            echo !empty($link)
                ? '</a>'
                : '</span>';
            ...
            ?><img
            src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'fitline'); ?>"<?php echo(!empty($attr[3]) ? ' ' . trim($attr[3]) : ''); ?>><?php
            echo !empty($link)
                ? '</a>'
                : '</span>';
        }
		...
		<div class="sc_services_item_header">
            ...
            ?><div class="sc_services_item_title_wrap">
                ...
                <h4 class="sc_services_item_title entry-title<?php if (!$price_showed && !empty($meta['price'])) echo ' with_price'; ?>"><?php
                    ...
                    ?><span class="sc_services_item_number"><?php printf("%02d" . '.', $number);?></span><span class="sc_services_item_title_text"><?php the_title();?></span><?php
