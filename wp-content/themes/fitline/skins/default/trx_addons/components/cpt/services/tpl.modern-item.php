<?php
/**
 * The style "modern" of the Services
 *
...
// SVG
$svg_bg_1 = trx_addons_get_svg_from_file(fitline_get_file_dir('images/svg_bg_1.svg'));
$svg_bg_2 = trx_addons_get_svg_from_file(fitline_get_file_dir('images/svg_bg_2.svg'));
$svg_bg = ($svg_bg_1 ? '<span class="svg-1">'.$svg_bg_1.'</span>' : '').($svg_bg_2 ? '<span class="svg-2">'.$svg_bg_2.'</span>' : '');
...
?>>
	...
	if ($args['featured'] != 'none') {
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters( 'trx_addons_filter_args_featured', array(
																...
																'thumb_size' => ! empty( $args['thumb_size'] )
																					? $args['thumb_size']
																					: fitline_get_thumb_size('masonry-big'),
																'post_info' => apply_filters('trx_addons_filter_post_info',
																				( ! empty($meta['price']) 
																					? '<span class="sc_services_item_price">'.trim($meta['price']).'</span>'
																					: ''
																					)
																				.(( ! empty( $svg_bg) && $args['featured_position']=='top' ) ? '<div class="all-svg">'.$svg_bg.'</div>' : ''),
																				'services-modern', $args  )
																),
															   'services-modern', $args
											)
										);
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" alt="<?php esc_attr_e('Icon', 'fitline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
		} else {
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			...
			if (!empty($link)) {
				?><div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>"<?php if (!empty($meta['link']) && trx_addons_is_external_url($meta['link'])) echo ' target="_blank"'; ?> class="sc_button_arrow"></a></div><?php
			}
		}
