<?php
/**
 * The style "minimal" of the Services item
 *
...
if ( has_post_thumbnail() ) {
	$image = trx_addons_get_attachment_url( 
											...
											! empty( $args['thumb_size'] )
												? $args['thumb_size']
												: apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('masonry-big'), 'services-minimal')
											);
}
...
?>>
    ...
    <?php do_action( 'trx_addons_action_services_item_content_start', $args ); ?>
         ...
         <div class="sc_services_item_featured_image<?php echo 1 == $args['hide_bg_image'] ? ' without_image' : ''; ?>"<?php if ($args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>></div>

         <div class="sc_services_item_content_inner"><?php
         do_action( 'trx_addons_action_services_item_content_inner_start', $args ); ?>
             <div class="sc_services_item_content_inner_center"><?php
