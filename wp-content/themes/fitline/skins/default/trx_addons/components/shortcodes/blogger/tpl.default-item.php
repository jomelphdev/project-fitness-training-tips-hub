...
if ( isset($templates[$args['type']][$args['template_'.$args['type']]]) ) {
	...
	foreach( $template_parts as $k=>$v ) {
		...
		if ( substr( $k, 0, 4 ) == 'meta') {
			...
			if ( false && (
				($args['type'] == 'default' && $args['template_default'] == 'over_centered_hover_2') ||
				($args['type'] == 'default' && $args['template_default'] == 'over_centered_hover_3') ||
                ($args['type'] == 'default' && $args['template_default'] == 'over_bottom') ||
                ($args['type'] == 'default' && $args['template_default'] == 'over_centered_hover')
                )
            ) {
				$title = get_the_title();
				if(!empty($title)) {
					...
					echo fitline_strwords($title, 13);
					fitline_show_layout((!empty($post_link) ? '</a>' : '') . '</h6>');
				}
			} else {
		...
		} else if ($k == 'courses_meta') {
			if ( $args['type'] == 'default' && $args['template_default'] == 'courses' && $args['post_type'] === 'lp_course' && fitline_exists_learnpress() ) {
				$course       = learn_press_get_course();
				$get_lessons  = $course->count_items( LP_LESSON_CPT );
				$get_students = $course->count_students();
				$lessons      = sprintf( _n( '%d lesson', '%d lessons', $get_lessons, 'fitline' ), $get_lessons );
				$students     = sprintf( _n( '%d student', '%d students', $get_students, 'fitline' ), $get_students );
				$template_parts[$k] =
                    '<div class="sc_blogger_item_course_meta">'
				        .'<div class="course_meta_item post_meta_lesson">' . esc_html( $lessons ) . '</div>'
                        .'<div class="course_meta_item post_meta_student">' . esc_html( $students ) . '</div>'
				    . '</div>';
			}
		} else if ($k == 'courses_price') {
			if ( $args['type'] == 'default' && $args['template_default'] == 'courses' && $args['post_type'] === 'lp_course' && fitline_exists_learnpress() ) {
                $course       = learn_press_get_course();
                $price_html   = $course->get_course_price_html();
                if ( $price_html ) {
                $template_parts[$k] = '<div class="sc_blogger_item_course_price">' . $course->get_course_price_html() . '</div>';
                }
			}
		} else {
		...
		?><div class="sc_blogger_item_body"><?php
			...
			if ( isset($template['layout']['featured']) ) {
				...
				$featured_args = array(
									...
									'thumb_ratio' => $args['image_ratio'] == 'none' ? ($args['columns'] > 2 ? '71:53': '840:473') : $args['image_ratio'],
