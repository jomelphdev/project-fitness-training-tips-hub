...
?>><?php
	...
	foreach ($args['icons'] as $item) {
		...
		?><div class="sc_icons_item<?php echo esc_attr(( !empty($item['link']) ) ? ' sc_icons_item_linked' : '');
		                                 echo esc_attr(( !empty($item['link_text']) ) ? ' with_more' : ''); ?>"><?php

            ?><span class="sc_icons_item_number"><?php printf("%02d", $number); ?></span><?php

            if (!empty($item['char']) || !empty($item['image']) || !empty($icon) || !empty( $item['title'] ) ) { ?>
                <div class="sc_icons_item_header"><?php
                    if (!empty($item['char'])) {
                        ...
                        $item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('medium'), 'icons-number'));
                        if (!empty($item['image'])) {
                            $attr = trx_addons_getimagesize($item['image']);
                            ?><div class="sc_icons_image"<?php trx_addons_sc_show_attributes('sc_icons', $args, 'sc_item_wrapper'); ?>><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'fitline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
                        }
                    } else {
                        ...
                        if (!empty($icon)) {
                            ...
                            ?>><?php
                                if (!empty($svg)) {
                                    ...
                                    ?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr_e('Icon', 'fitline'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
                                } else {
			...
			if ( ! empty( $item['description'] ) || ! empty( $item['link'] ) ) {
				?><div class="sc_icons_item_details"><?php
                    ...
                    if ( ! empty( $item['link'] &&  !empty($item['link_text'] ) ) ) {
                        ?><a href="<?php echo esc_url($item['link']); ?>" class="sc_icons_item_more_link"<?php
                            if (!empty($item['new_window']) || !empty($item['link_extra']['is_external'])) echo ' target="_blank"';
                            if (!empty($item['nofollow']) || !empty($item['link_extra']['nofollow'])) echo ' rel="nofollow"';
                        ?>>
                        <span class="link_text"><?php echo esc_html($item['link_text']);?></span><span class="link_icon"></span>
                        </a><?php
                    }
