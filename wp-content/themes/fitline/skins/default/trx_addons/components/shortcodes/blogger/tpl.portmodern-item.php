...
$details     = '';
if ( 'cpt_portfolio' == $args['post_type'] ) {
    ob_start();
	...
	foreach( $template_parts as $k=>$v ) {
		...
		if ( substr( $k, 0, 4 ) == 'meta') {
			...
			the_title( '<h1 class="sc_blogger_item_title entry-title"'
                        ...
                        . ( !empty($image) ? ' data-item-image="' . esc_attr($image[0]) . '"' : '' )
						. ' data-item-id="' . get_the_ID() . '_wrap' . '"'
                        . ' data-details="' . esc_attr( '<div class="post_details">'
                            . '<h2 class="post_title">'
                            . '<a href="' . esc_url( get_permalink() ) . '">' . esc_html( get_the_title() ) . '</a>'
                            . '</h2>'
                            . '<div class="post_description">'
                            . $details
                            . ( empty( $fitline_template_args['hide_excerpt'] )
                                ? '<div class="post_description_content">' . get_the_excerpt() . '</div>'
                                : ''
                            )
                            . '<a href="' . esc_url( get_permalink() ) . '" class="theme_button post_readmore"><span class="post_readmore_label">' . (!empty($args['more_text']) ? esc_html($args['more_text']) : esc_html__( 'Learn more', 'fitline' )) . '</span></a>'
                            . '</div>'
                            . '</div>'
                        ) . '"'
						. '>',
