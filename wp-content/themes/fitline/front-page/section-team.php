<div class="front_page_section front_page_section_team<?php
	$fitline_scheme = fitline_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $fitline_scheme ) && ! fitline_is_inherit( $fitline_scheme ) ) {
		echo ' scheme_' . esc_attr( $fitline_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( fitline_get_theme_option( 'front_page_team_paddings' ) );
	if ( fitline_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$fitline_css      = '';
		$fitline_bg_image = fitline_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$fitline_anchor_icon = fitline_get_theme_option( 'front_page_team_anchor_icon' );
	$fitline_anchor_text = fitline_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( fitline_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$fitline_css      = '';
			$fitline_bg_mask  = fitline_get_theme_option( 'front_page_team_bg_mask' );
			$fitline_bg_color_type = fitline_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $fitline_bg_color_type ) {
				$fitline_bg_color = fitline_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$fitline_caption = fitline_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $fitline_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $fitline_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $fitline_caption, 'fitline_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$fitline_description = fitline_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $fitline_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $fitline_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $fitline_description ), 'fitline_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! fitline_exists_trx_addons() ) {
						...
						fitline_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
