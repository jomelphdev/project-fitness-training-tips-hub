...
if ( ! class_exists( 'Utils' ) ) {
	...
	class Utils {
		...
		static function get_default_max_tokens() {
			return apply_filters( 'trx_addons_filter_ai_helper_default_max_tokens', 32000 );
		}
		...
		static function is_api_available( $sc ) {
			$rez = false;
			$api_order = trx_addons_get_option( "ai_helper_{$sc}_api_order", $sc == 'sc_igenerator' ? Lists::get_list_ai_chat_apis_enabled() : Lists::get_list_ai_image_apis_enabled() );
			foreach( $api_order as $api => $enable ) {
		 ...
		 */
		static function is_chat_api_available() {
			return self::is_api_available( 'sc_chat' );
		}
