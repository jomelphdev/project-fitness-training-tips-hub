...
class ListCoursesTemplate {
	...
	public static function render_courses( array $settings = [] ): stdClass {
		...
		ob_start();
		if ( empty( $courses ) ) {
			echo sprintf( '<p class="learn-press-message success">%s!</p>', __( 'No videos found', 'learnpress' ) );
		} else {
	...
	public function html_search_form( array $data = [] ) {
		...
		<form class="search-courses" method="get"
			...
			<label>
				<input type="text" placeholder="<?php esc_attr_e( 'Search videos...', 'learnpress' ); ?>"
						name="c_search"
						value="<?php echo esc_attr( $s ); ?>">
			</label>
			<button type="submit" name="lp-btn-search-videos"><i class="fas fa-search"></i></button>
		</form>
		<?php
		return ob_get_clean();
	}
